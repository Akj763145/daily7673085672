    ...
    (set, get) => ({
      ...
      toggleDelivery: (date, itemId) => {
        ...
        set((state) => {
          ...
          const current = dayDeliveries[itemId] || { delivered: false, adjustment: 0 };
        ...
        set((state) => {
          ...
          return {
            deliveries: {
              ...
              [dateKey]: {
                ...dayDeliveries,
                [itemId]: { ...current, adjustment: sanitizedAmount },
              },
            },
          };
        });
      ...
      getAdjustment: (date, itemId) => {
        const dateKey = format(date, 'yyyy-MM-dd');
        const delivery = get().deliveries[dateKey]?.[itemId];
        const adj = delivery?.adjustment ?? 0;
      ...
      calculateBill: (startDate, endDate) => {
        ...
        while (currentDate <= lastDate) {
          ...
          items.forEach((item) => {
            const delivery = dayDeliveries[item.id] || { delivered: false, adjustment: 0 };
            const isDelivered = delivery.delivered;
            const adjustment = isNaN(delivery.adjustment) ? 0 : delivery.adjustment;
            ...
            if (isDelivered) {
              ...
              const price = priceEntry ? (isNaN(priceEntry.price) ? 0 : priceEntry.price) : 0;
